...
plot.log.state = function(dat,par,obj,opt,map,np = 4)
{
    ...
    err = get.error(par,opt,map,'sigma_logC')
    ...
    err = get.error(par,opt,map,'sigma_logbeta')
    logspace.plot.error(tt,obj$report()$logbeta,err)
    gmbeta = mean(obj$report()$logbeta)
    ...
    err = get.error(par,opt,map,'sigma_logD')
    ...
    err = get.error(par,opt,map,'sigma_logmu')
    logspace.plot.error(tt,obj$report()$logmu,err)
    gmmu = mean(obj$report()$logmu)
