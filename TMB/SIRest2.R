#rm(list=ls())
#gc(verb=TRUE)
eps = 1e-8
source('./SIR_read_dat.R')
print(" ",quote=FALSE)
print("##############################################",quote=FALSE)
SIR.dat = read.dat.file('SIRest2.dat')
print(names(SIR.dat))
data = SIR.dat$data
print("----data:")
print(data)
# from python simulator
# def SIRsim(mu = 0.001, beta = 0.5, gamma = 0.075, N0 = 1e5, sigma_eye = 0.3,

init = list(
    sigma_P = 20.0,
    sigma_beta = 0.05,
    mu = 0.2,
    gamma = 0.5,
    sigma_C = 10.0,
    sigma_D = 2.0,
    beta = 0.13
)
print("-initial values:")
print(init)

parameters = list(
    logsigma_P = log(init$sigma_P),
    logsigma_beta = log(init$sigma_beta),
    logmu    = log(init$mu),
    loggamma = log(init$gamma),
    logsigma_C = log(init$sigma_C),
    logsigma_D = log(init$sigma_D),
    logbeta = rep(log(init$beta),data$ntime)
)
#   S = rep(data$N0,data$ntime),
#   R = rep(eps,data$ntime)
print(paste("--parameters: ", length(parameters)))
print(parameters)

map = list("logsigma_P" = as.factor(NA),
           "logsigma_C" = as.factor(NA),
           "logsigma_D" = as.factor(NA),
           "logsigma_beta" = as.factor(NA))
#          "loggamma"  = as.factor(NA),
#          "logmu"  = as.factor(NA),
print(paste("---map: ",length(map)))
print(map)

require(TMB)

print("Compiling-------------------------------------",quote=FALSE)
compile("SIRest2.cpp")
dyn.load(dynlib("SIRest2"))
print("Finished compilation and dyn.load-------------",quote=FALSE)
print("Calling MakeADFun-----------------------------",quote=FALSE)
obj = MakeADFun(data,parameters,random=c("logbeta"),map=map,DLL="SIRest2")
#obj = MakeADFun(data,parameters,map=map,DLL="SIRest2")
print("--------MakeADFun Finished--------------------",quote=FALSE)
print(paste("par: ",obj$par))

print("Starting minimization-------------------------",quote=FALSE)
obj$control=list(trace=1,eval.max=1,iter.max=2)
opt <- nlminb(obj$par,obj$fn,obj$gr,control=obj$control)
##opty= nlminb(mody $par, mody $fn, mody $gr)
print("Done minimization-----------------------------",quote=FALSE)
print(paste("Objective function value =",opt$objective))
print(paste("Number of parameters = ",length(opt$par)),quote=FALSE)
print("parameters:",quote=FALSE)
print(opt$par)
print(exp(opt$par))
gmbeta = exp(mean(obj$report()$logbeta))
print(paste("geometric mean beta:",gmbeta))

plot.state(data,obj)
