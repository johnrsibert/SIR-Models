...
template <class Type>
Type ZILNerr(Type obs, Type pred, Type var, Type prop0 = 0.15)
{
    Type tmp;
...
Type objective_function <Type>::operator()()
{
  ...
  //PARAMETER_VECTOR(S);            // susceptible population
    vector <Type> R(ntime);         // number of recovered
  //PARAMETER_VECTOR(R);            // number of recovered
  ...
  //Type gamma = exp(loggamma);
    ...
    for (int t = 1; t <  ntime; t++)
    {
         ...
         Type betanll = 0.0;
         betanll += 0.5*(log(TWO_M_PI*varlogbeta)
                   + square(logbeta(t-1) -logbeta(t))/varlogbeta);

         Type Pnll = 0.0;
         N(t) = S(t-1) + Eye(t-1) + R(t-1);

         Type bison = exp(logbeta(t-1))*Eye(t-1)/(N(t-1)+eps);
         //Type gEye = exp(loggamma)*Eye(t-1);
         //Type mEye = exp(logmu)*Eye(t-1);

         Eye(t) = Eye(t-1) + (exp(logbeta(t-1))*Eye(t-1)*S(t-1)/N(t-1))
                           - exp(logmu)*Eye(t-1) - exp(loggamma)*Eye(t-1);
         ...
         R(t) = R(t-1) + exp(loggamma)*Eye(t-1) -exp(logmu)*R(t-1);
         Pnll += NLerr(R(t-1)+eps, R(t)+eps, varlogPop);

         D(t) = exp(logmu)*Eye(t-1);
         Pnll += NLerr(D(t-1)+eps, D(t)+eps, varlogPop);
     ...
     {   
         dnll += ZILNerr(obs_deaths(t),D(t),varlogdeaths, prop_zero_deaths);
