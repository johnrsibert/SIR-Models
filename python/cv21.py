...
#tgeog.print_data()
#tgeog.plot_prevalence0(save=False, signature=True,show_superspreader=False)
...
def plot_prevalence(yscale='linear', per_capita=False, delta_ts=True,
...
#   print(gg_filter)
    ...
    for g in range(0,len(gg)):
        ...
        #   print(a,any(pd.isna(gdf.iloc[:,a])))
            if (any(pd.isna(gdf.iloc[:,a]))):
                do_plot[a] = False
        #   else:
        #       ymax[a] = max(ymax[a],gdf.iloc[:,a].max())
        #   print(ymax)
        ...
        for a in range(0,nax):
            ...
            #   ax[a].bar(Date[1:], delta)
                ...
                #   GU.mark_ends(ax[a],Date[1:],adc,GG.short_name(tmpG.moniker),'r')
...
#               print(ylim[a])
                ...
                GU.mark_ends(ax[a],Date,gdf.iloc[:,a],GG.short_name(tmpG.moniker),'r')
...
#       ax[0] = GU.plot_dtslopes(ax[0],Date,gdf.iloc[:,0])
    ...
    if (annotation):
        ...
        GU.add_data_source(fig,tmpG.source)
    ...
    #   return "data:image/png;base64,{}".format(encoded)
        ...
        if save:
            gfile = cv.graphics_path+'prevalence_comp'+str(nG)+'.png'
            plt.savefig(gfile,dpi=300)
            plt.show(block=False)
            plt.pause(3)
            plt.close()
...
plot_prevalence(save=True, signature=False,show_superspreader=False, per_capita=False)
